...
export default function PSAPScorecardPage() {
  ...
  const getRatingColor = (rating: string | null) => {
    const scale = ratingScales.find(s => s.rating_label === rating)
    switch (scale?.color_code) {
      case 'green':
        return 'bg-green-500'
      case 'lime':
        return 'bg-lime-500'
  ...
  const handleYearChange = (year: string) => {
    ...
    const quarter = selectedQuarter ? parseInt(selectedQuarter) : undefined
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {assessments.filter(a => a.overall_rating === 'Full Compliance').length}
            </div>
                ...
                <TableBody>
                  {standards.map((standard) => (
                    <TableRow key={standard.id}>
                      <TableCell className="font-medium">{standard.standard_no}</TableCell>
                      <TableCell>
                        <div>
                      ...
                      <TableCell className="font-mono">{standard.weighting.toFixed(1)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            <CardContent>
              <div className="space-y-3">
                {ratingScales.map((scale) => (
                  ...
                  >
                    <div className={`h-4 w-4 rounded-full ${getRatingColor(scale.rating_label)}`} />
                    <div className="flex-1">
                      <div className="font-medium">{scale.rating_label}</div>
