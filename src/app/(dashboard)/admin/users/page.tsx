...
export default function UsersPage() {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      // Generate a valid UUID v4 for the user. audit_users.id is a uuid column,
      // so the fallback must also be a real UUID (not a timestamp string).
      // Prefer crypto.randomUUID when available (secure contexts), otherwise
      // build a v4 UUID from Math.random for broad compatibility.
      const userId =
      ...
      <PageContainer>
        ...
        <CardContent className="p-4">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <HelpTooltip tooltipKey="admin-users-search" label="Search users" />
