...
type PSAPStandard = Database['public']['Tables']['psap_standards']['Row']
type PSAPAssessment = Database['public']['Tables']['psap_assessments']['Row']
type PSAPAssessmentInsert = Database['public']['Tables']['psap_assessments']['Insert']
type PSAPAssessmentScore = Database['public']['Tables']['psap_assessment_scores']['Row']
type PSAPAssessmentScoreInsert = Database['public']['Tables']['psap_assessment_scores']['Insert']
type PSAPRatingScale = Database['public']['Tables']['psap_rating_scales']['Row']
    ...
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          org_units(name, code),
          completed_by_person:people!psap_assessments_completed_by_fkey(full_name),
          reviewed_by_person:people!psap_assessments_reviewed_by_fkey(full_name)
        `)
  ...
  const fetchAssessmentScores = async (assessmentId: string) => {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          psap_standards(standard_no, title, description, weighting, category)
        `)
        .eq('psap_assessment_id', assessmentId)
        .order('psap_standards(standard_no)', { ascending: true })
