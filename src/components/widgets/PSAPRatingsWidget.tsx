...
export function PSAPRatingsWidget() {
  ...
  const [ratings, setRatings] = useState<PSAPRating[]>([])
  ...
  const loadRatings = async () => {
    try {
      ...
      if (error) {
        ...
        // If view doesn't exist, try fetching from psap_assessments directly
        ...
        setRatings(assessmentsData?.map(a => ({
          ...(a as Record<string, unknown>),
          org_unit_name: (a as { org_units?: { name?: string } }).org_units?.name
        })) as PSAPRating[] || [])
        return
      }
      setRatings(data || [])
