...
export function PendingRiskEventsWidget() {
  ...
  const fetchPendingEvents = async () => {
    try {
      ...
      if (allEvents) {
        const typedEvents = allEvents as unknown as Array<{ event_status: string; source_type: string }>
        const pending = typedEvents.filter(e => e.event_status === 'Pending').length
        const underReview = typedEvents.filter(e => e.event_status === 'Under Review').length
        const approved = typedEvents.filter(e => e.event_status === 'Approved').length
        const secretaryDirectives = typedEvents.filter(e => e.source_type === 'Secretary').length
