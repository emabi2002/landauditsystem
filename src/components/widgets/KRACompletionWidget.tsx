...
export function KRACompletionWidget() {
  ...
  const loadSummary = async () => {
    try {
      ...
      // Query from actual tables since view doesn't exist yet
        ...
        .select(`
          *,
          audit_kra_activities(
            *,
            audit_strategic_kras(kra_code, kra_title)
          )
        `)
      ...
      if (data && data.length > 0) {
        const kraGroups: Record<string, any> = {}
        ;(data as any[]).forEach((status: any) => {
          const kra = status.audit_kra_activities?.audit_strategic_kras
          if (!kra) return

          if (!kraGroups[kra.kra_code]) {
            kraGroups[kra.kra_code] = {
          ...
          kraGroups[kra.kra_code].total_activities++
          if (status.status === 'Completed') kraGroups[kra.kra_code].completed++
          if (status.status === 'Ongoing') kraGroups[kra.kra_code].ongoing++
          if (status.status === 'Delayed') kraGroups[kra.kra_code].delayed++
        })

        const items = Object.values(kraGroups)
        const totals = {
          total_activities: items.reduce((sum: number, d: any) => sum + (d.total_activities || 0), 0),
          completed: items.reduce((sum: number, d: any) => sum + (d.completed || 0), 0),
          ongoing: items.reduce((sum: number, d: any) => sum + (d.ongoing || 0), 0),
          delayed: items.reduce((sum: number, d: any) => sum + (d.delayed || 0), 0),
        }

        setSummary({ items, totals, year: currentYear, quarter: currentQuarter })
      }
    } catch (error) {
      console.error('Error loading KRA summary:', error)
      setSummary(null)
