  ...
  const loadSummary = async () => {
    try {
      ...
      if (error) {
        ...
        setSummary({ items: [], totals: { total_risks: 0, extreme_risks: 0, high_risks: 0, open_risks: 0 } })
        setLoading(false)
        return
      }

      // For now, set simple placeholder data since views don't exist
      const totals = {
        total_risks: data?.length || 0,
        extreme_risks: 0,
        high_risks: 0,
        open_risks: data?.length || 0,
      }
